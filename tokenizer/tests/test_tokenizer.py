...
# test encode/decode identity for a few different strings
@pytest.mark.parametrize("text", test_strings)
def test_encode_decode_identity_hf(text):
    text = unpack(text)
    tokenizer = Tokenizer.load_from_hf('hf_commit-merges.txt', 'hf_commit-vocab.json')
    ids = tokenizer.encode(text)
    decoded = tokenizer.decode(ids)
    assert text == decoded
...
def test_save_load(special_tokens):
    ...
    tokenizer.register_special(special_tokens)
    ...
    tokenizer = Tokenizer.load_from_file(path)
    # verify that decode(encode(x)) == x
